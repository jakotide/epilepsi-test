...
export function createPaintJourney(paperUniforms: Record<string, THREE.IUniform>) {
  ...
  const material = new THREE.ShaderMaterial({
    ...
    uniforms,
    vertexShader: `varying vec2 vUv;uniform float uDirection,uPortrait;
      void main(){vUv=uv;vec3 p=position;p.x*=uDirection;p.y+=uPortrait*uv.y*1.4;
        gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.);}`,
    fragmentShader: /* glsl */ `${paperShader}
      varying vec2 vUv;uniform float uProgress,uOpacity;
      float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}
      float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);
        return mix(mix(hash(i),hash(i+vec2(1,0)),f.x),mix(hash(i+vec2(0,1)),hash(i+vec2(1,1)),f.x),f.y);}
      void main(){
        vec2 uv=vUv;
        float bloom=noise(uv*vec2(13.,43.))*.65+noise(uv*vec2(36.,110.))*.35;
  ...
  return {
    scene, root, uniforms,
    reset(){root.visible=false;uniforms.uProgress.value=0;uniforms.uOpacity.value=0;},
